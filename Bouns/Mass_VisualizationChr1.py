...
Merged_masses.to_csv("Origenal_Mutated_Masses.csv", index = False)
...
results_dir = os.path.join(script_dir, 'MassVisualized_Graphs/')
...
for i in range(0,iterations):
    ...
    if i == iterations-1:
        ...
        plt.savefig(results_dir + 'MassDifference_Figure{}.png'.format(i+1), dpi=300, bbox_inches='tight')
        plt.close()
        break
...
results_dir = os.path.join(script_dir, 'MassChange_Graphs/')
if not os.path.isdir(results_dir):
    os.makedirs(results_dir)
...
for i in range(0,iterations):
    ...
    if i == iterations-1:
        ...
        plt.xlabel('Gene Names', fontsize=12)
        plt.savefig(results_dir + 'ChangeInMassRelationship_Figure{}.png'.format(i+1), dpi=300, bbox_inches='tight')
        plt.close()
        break
